...
class JsonDecoder:
   ...
   def decode(self):
       ...
       if token == '{':
           self._decode(self._class_name, path, self._predicate.split('.') if self._predicate and len(self._predicate) > 0 else list(), self._handler)
       elif token == '[':
           self._decode_list(self._class_name, path, self._predicate.split('.') if self._predicate and len(self._predicate) > 0 else list(), self._handler)
       ...
       while self._tokenizer.has_next():
           ...
           if prop_value == '{':
               if self._path_matches(path, predicate):
                   ...
                   elements = list()
                   self._decode(self.to_pascal_case(prop_name), path, predicate, lambda e, p: self._record_result(elements, e))
               ...
               if self._path_matches(path, predicate):
                   ...
                   elements = list()
                   self._decode_list(self.to_pascal_case(prop_name), path, predicate, lambda e, p: self._record_result(elements, e))
                   prop_value = elements
               else:
                   self._skip_list()
           else:
   ...
   def _path_matches(self, path, predicate):
       return True if len(predicate) > 0 and len(predicate) >= len(path) and path[-1] == predicate[len(path)-1] else False
